...
{
    [Header("Settings")] [Tooltip("The event to call on collisions")]
    public UnityEvent collisionEvent = new();
    ...
    {
        if (requiredTag != "") return tag == requiredTag;
        return true;
    }
    ...
    {
        if (TestLayer(caller.layer) && TestTag(caller.tag)) collisionEvent.Invoke();
    }
}