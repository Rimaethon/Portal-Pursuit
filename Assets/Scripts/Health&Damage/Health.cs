...
public class Health : MonoBehaviour
{
    ...
    {
        if (timeToBecomeDamagableAgain <= Time.time) isInvincible = false;
    }
    ...
    {
        if (isInvincible || currentHealth <= 0 || isAlwaysInvincible) return;
    ...
    {
        if (GameManager.instance != null && gameObject.tag == "Player") GameManager.instance.GameOver();
    }
}