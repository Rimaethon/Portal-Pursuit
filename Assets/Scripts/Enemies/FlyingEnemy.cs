...
public class FlyingEnemy : Enemy
{
    ...
    [Header("Flying Enemy Settings")] [Tooltip("The distance at which point the stop behavior is engaged")]
        ...
        if (target != null)
        {
            ...
            switch (stopBehavior)
            {
                ...
                case BehaviorAtStopDistance.CircleClockwise:
                    return transform.position + Vector3.Cross(target - transform.position, transform.up).normalized *
    ...
    {
        if (target != null) return Quaternion.LookRotation(target - transform.position, Vector3.up);
