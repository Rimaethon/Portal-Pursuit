...
{
    [Header("Ground Enemy Settings")] [Tooltip("The nav mesh agent used to move this enemy")]
    ...
    [Tooltip(
        "Whether this enemy can stop if it is within it's stop distance but does not have line of sight to it's target")]
        ...
        bool ShouldMove()
        {
            if (agent != null && target != null && canMove)
            {
                if ((target - transform.position).magnitude > stopDistance)
                    return true;
                if (awareness != null && lineOfSightToStop && !awareness.CheckLineOfSight()) return true;
            }
    ...
    protected override Quaternion CalculateDesiredRotation()
    {
        if (alwaysFacePlayer)
        {
            if (awareness != null && awareness.certaintyOfPlayer > awareness.followThreshold)
            {
                ...
                toTarget.y = 0;
                var result = Quaternion.LookRotation(toTarget, transform.up);
                return result;
            }
