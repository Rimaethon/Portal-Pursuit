...
public class PlayerRepresentation : MonoBehaviour
{
    ...
    [Header("Following Settings")] [Tooltip("An offeset to follow at")]
    ...
    private void CentralizedControl(float leftCharacterMovement, float rightCharacterMovement)
    {
    ...
    /// <param name="leftCharacterMovement">The movement input along the horizontal</param>
    /// <param name="rightCharacterMovement">The movement input along the vertical</param>
    private void NormalControl(float leftCharacterMovement, float rightCharacterMovement)
    {
        var movementDirection = new Vector3(leftCharacterMovement, 0, rightCharacterMovement);
        if (movementDirection != Vector3.zero)
        {
            var rotation = Quaternion.LookRotation(movementDirection, Vector3.up);
            rotation = Quaternion.Euler(rotation.eulerAngles.x,
                playerCharacterController.transform.rotation.eulerAngles.y + rotation.eulerAngles.y,
    ...
    {
        if (representationAnimator == null) return;

        // Get the player state
        var currentPlayerState = playerCharacterController.playerState;
