...
{
    [Header("Needed References")] [Tooltip("The player character controller")]
    public CharacterController player;

    [Tooltip("This is what the camera looks at, can be the player or any another gameobject")]
    public GameObject lookAt;

    [Header("Speed Controls")] [Tooltip("How quickly the camera moves horizontally")]
    ...
    [Header("Distance Controls")] [Tooltip("Maximum distance away from the player")]
    ...
    [Header("Linear interpolation smoothing")] [Tooltip("The amount of smoothing to apply when moving the camera")]
    public float smoothing = 10f;

    [Header("Raycast settings")]
    [Tooltip(
        "The Distance to offset from the collision (Should be slightly less than the players xz scale to avoid moving through the player up close)")]
    ...
    private void InitialSetup()
    {
        ...
        if (PlayerPrefs.HasKey("InvertX"))
        {
            ...
            else if (PlayerPrefs.GetInt("InvertX") == 0) SetInvertedHorizontal(false);
        }
        ...
        if (PlayerPrefs.HasKey("InvertY"))
        {
            ...
            else if (PlayerPrefs.GetInt("InvertY") == 0) SetInvertedVertical(false);
        }
        ...
        if (rotationReference == null)
            Debug.LogError(
                "The third person camera is not a child of a gameobject with a CameraRotationHelper script attached \n" +
    ...
    private void CentralizedControl(float horizontalLook, float verticalLook)
    {
        // Camera input
        ...
        var horizontalRotationAdjustment = horizontalInput * cameraHorizontalSpeed * Time.deltaTime;
        // How much to adjust the rotation vertically
        var verticalRotationAdjustment = verticalInput * cameraVerticalSpeed * Time.deltaTime;
        ...
        if (horizontalRotationAdjustment != 0)
            ...
            rotationToChangeToInEular = new Vector3(rotationToChangeToInEular.x,
                rotationToChangeToInEular.y + horizontalRotationAdjustment, rotationToChangeToInEular.z);
        ...
        if (verticalRotationAdjustment != 0)
            ...
            rotationToChangeToInEular = new Vector3(rotationToChangeToInEular.x + verticalRotationAdjustment,
                rotationToChangeToInEular.y, rotationToChangeToInEular.z);
        ...
        if (rotationToChangeToInEular.x > maximumXRotation && rotationToChangeToInEular.x < 180)
            rotationToChangeToInEular =
                new Vector3(maximumXRotation, rotationToChangeToInEular.y, rotationToChangeToInEular.z);
        if (rotationToChangeToInEular.x < minimumXRotation + 360 && rotationToChangeToInEular.x >= 180)
            rotationToChangeToInEular =
                new Vector3(minimumXRotation, rotationToChangeToInEular.y, rotationToChangeToInEular.z);
    ...
    /// <summary>
    ///     Description:
    ///     Uses raycasting to determine adjustments and then makes those adjustments to keep the camera infront of objects in
    ///     the world
    ...
    ///     void
    /// </summary>
    private void RaycastAdjustments()
    {
        ...
        var lookAtPosition = lookAt.transform.position;
        ...
        var directionToCameraFromLookAt = (cameraPostion - lookAtPosition).normalized;

        // The current x rotation
        var currentXRotation = rotationReference.rotation.eulerAngles.x;

        // If the current x rotation is greater than 180, convert it to be a negative degree
        if (currentXRotation > 180) currentXRotation = currentXRotation - 360;
        ...
        var topOfRange = Mathf.Abs(xRotationValueWhereMaxDistanceBecomesMinimum) +
                         Mathf.Abs(xRotationValueWhereMaxDistanceIsMaximum);

        // Calculate the ratio of the current X rotation plus the maximum rotation over the ranges maximum
        var ratio = (currentXRotation + xRotationValueWhereMaxDistanceIsMaximum) / topOfRange;
        ...
        var moveForwardCastHit = new RaycastHit();
        // Move forward cast
        var distanceToCast = (transform.position - lookAt.transform.position).magnitude;
        ...
        var idleCastHit = new RaycastHit();
        // Idle (dont move cast)
        ...
        radio = gameObject.AddComponent<RadioInteractor>();
        // If we hit something in the move forward ray cast position the camera accordingly
        if ((moveForwardCastHit.transform != null) & radio.radioHit)
        {
            transform.position = Vector3.Lerp(transform.position,
                lookAtPosition + directionToCameraFromLookAt *
                Mathf.Clamp(moveForwardCastHit.distance - offsetDistanceFromCollision, minimumDistance,
                    appliedMaximumDistanceAway), Time.deltaTime * smoothing);
        }
        ...
        else
        {
            ...
            var centerCameraHit = new RaycastHit();
            var rayCastDirection = (transform.position - lookAt.transform.position).normalized;
            Physics.Raycast(lookAt.transform.position, rayCastDirection, out centerCameraHit,
                appliedMaximumDistanceAway);
            if (centerCameraHit.transform != null)
                transform.position = Vector3.Lerp(transform.position,
                    centerCameraHit.point - rayCastDirection * offsetDistanceFromCollision, Time.deltaTime * smoothing);
            else
                transform.position = Vector3.Lerp(transform.position,
                    lookAtPosition + directionToCameraFromLookAt * appliedMaximumDistanceAway,
    ...
    private RaycastHit RaycastFromLookAtToPoint(Transform point, float distanceToCast)
    {
        RaycastHit results;
        var rayCastDirection = (point.position - lookAt.transform.position).normalized;
    ...
    ///     void
    /// </summary>
    private void DetermineCameraPositionAdjustments()
    {
        // Determine if we should move backwards

        // whether or not the camera is within the xz bound
        var withinXZBound = false;
        // whether or not the camera is within the Y bound
        var withinYBound = false;

        // XZ movement check
        // the camera's positon along the X and Z
        var cameraXZPosition = new Vector3(transform.position.x, 0, transform.position.z);
        // The player's position along the X and Z
        var playerXZPosition = new Vector3(player.transform.position.x, 0, player.transform.position.z);
        // The direction from the player to the camera on the XZ plane
        var directionFromPlayerToCameraXZ = cameraXZPosition - playerXZPosition;
        ...
        if (directionFromPlayerToCameraXZ.magnitude < appliedMaximumDistanceAway) withinXZBound = true;

        // Y movement check
        var yDistanceBetweenPlayerAndCamera = player.transform.position.y - transform.position.y;
        if (yDistanceBetweenPlayerAndCamera < maximumDistanceOnYAxis &&
            yDistanceBetweenPlayerAndCamera > -maximumDistanceOnYAxis) withinYBound = true;
    ...
    /// <summary>
    ///     Description:
    ///     Uses raycasting to determine if the player is in the shot, if the player is not, makes the appropriate camera
    ///     positioning
    ///     Input:
    ///     none
    ///     Return:
    ///     void
    /// </summary>
    private void RaycastForPlayerVisibility()
    {
        var centerToCamera = RaycastFromGivenTransformToCamera(player.transform);
        var centerToCameraHit = centerToCamera.transform != null;

        RaycastHit backFromCameraMoreThanOffset;
        RaycastHit groundRayCastCheck;

        Physics.Raycast(transform.position, (transform.position - rotationReference.transform.position).normalized,
            out backFromCameraMoreThanOffset, offsetDistanceFromCollision * 2);
        ...
        if (centerToCameraHit)
        {
            ...
            var offsetCorrection = (rotationReference.transform.position - transform.position).normalized *
                                   offsetDistanceFromCollision;
            var snapToPosition = centerToCamera.point + offsetCorrection;
        ...
        {
            var modifyByDistance = 1 / (player.transform.position - transform.position).magnitude *
                                   maximumBackWardMovespeed;
            // Cap modifier to a maximum and minimum
            if (modifyByDistance > maximumBackWardMovespeed) modifyByDistance = maximumBackWardMovespeed;

            transform.Translate(-Vector3.forward * Time.deltaTime * modifyByDistance, Space.Self);
        }
    }

    private RaycastHit RaycastFromGivenTransformToCamera(Transform startingFrom)
    {
        RaycastHit results;
        var rayCastDirection = (transform.position - startingFrom.position).normalized;

        var distanceToCast = (transform.position - startingFrom.position).magnitude + offsetDistanceFromCollision;
